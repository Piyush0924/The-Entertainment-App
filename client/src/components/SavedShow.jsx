...
const SavedShow = () => {
  ...
  return (
    <>
      <h2 className="text-white font-bold md:text-xl p-4">Bookmarked Shows</h2>{" "}
      {/* Title for saved shows */}
        ...
        >
          ...
          {savedMovies.map((item) => (
            <div
              ...
              onClick={() =>
                handleClick(item.id, item.media_type || item.mediaType)
              } // Navigate on click
            >
              <img
              ...
              />
              <div className="absolute top-0 left-0 w-full h-full opacity-0 hover:opacity-100 text-white transition-opacity duration-300">
                <p
                  onClick={(e) => {
                    e.stopPropagation(); // Prevent triggering the click event for navigation
                    deleteShow(item.id);
                  }}
                  className="absolute top-4 right-4 p-2 rounded-full bg-red-950 hover:bg-red-800 hover:scale-110 transform transition-all duration-300 ease-in-out cursor-pointer"
