...
const MovieCard = ({ posterPath, movieId, title, backdropPath, mediaType }) => {
  console.log("title", title); // Log the title for debugging
      ...
      <div className='absolute top-0 left-0 w-full h-full hover:bg-black/80 opacity-0 hover:opacity-100 text-white'>
        ...
        <p onClick={(e) => { e.stopPropagation(); saveShow({ id: movieId, title, backdropPath, posterPath, mediaType }); }}>
          ...
          {savedMovies.some((savedMovie) => savedMovie.id === movieId) ? (
            <IoBookmark className='absolute top-4 right-6 text-gray-300' /> // Filled bookmark icon if movie is saved
          ) : (
            <IoBookmarkOutline className='absolute top-4 right-6 text-gray-300' /> // Outline bookmark icon if movie is not saved
          )}
        </p>
      </div>
